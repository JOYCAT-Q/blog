...
class GoogleOauthManager(ProxyManagerMixin, BaseOauthManager):
    ...
    def get_authorization_url(self, nexturl='/'):
        ...
        url = self.AUTH_URL + "?" + urllib.parse.urlencode(params)  # 将`params`字典转换为URL编码格式的查询字符串
        return url
...
def get_manager_by_type(type_oauth):
    applications = get_oauth_apps()
    if applications:
        finds = list(
            filter(
                lambda x: x.ICON_NAME.lower() == type_oauth.lower(),
                applications))
        if finds:
            return finds[0]
    return None
